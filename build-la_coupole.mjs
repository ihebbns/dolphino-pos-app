// ═══════════════════════════════════════════════════
// build-la_coupole.mjs — Rebuild the La Coupole EXE directly
// from clients/La_Coupole/index.html.
...
try {
  // 1) Bundle the current La Coupole HTML as the app index
  fs.copyFileSync(CLIENT_INDEX, ROOT_INDEX);
  console.log('✓ Copied clients/La_Coupole/index.html → index.html');

  // 2) Patch package.json build config for La Coupole
  ...
  pkg.build.appId                 = 'tn.servio.pos.lacoupole';
  ...
  // 4) Restore package.json and root index.html to whatever they were before
